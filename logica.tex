...
\begin{itemize}
  ...
  \item Dado un programa $P$, definimos el \xbf{conjunto de falla finita} de $P$ como \{$B$ | $B$ átomo cerrado ('ground') y existe un árbol SLD que falla finitamente con  $B$ como raiz \}
\end{itemize}
...
La clave está en combinar el operador \xtt{!} y \xtt{fail}. Una vez que el \xtt{Goal} tiene éxito, el cut nos prohibe de volver atras en el árbol SLD, y el \xtt{fail} hace que la solución actual falle. Como no hay más ramas que explorar, toda la consulta falla.
